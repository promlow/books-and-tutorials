...
Eval simpl in (LNth 15 (LAppend (repeat 33) (repeat 69))).
...
Eval simpl in (LList_decomp (repeat 33)).
...
Eval simpl in (LList_decomp (LAppend LNil (repeat 33))).

Eval simpl in (LList_decomp (LAppend (repeat 33) (repeat 69))).
...
Proof.
 intros A a; LList_unfold (repeat a); trivial.
Qed.
...
Hint Unfold satisfies: llists.
 ...
 Lemma Next_example : satisfies (LCons a (repeat b)) (Next (Atomic (eq b))).
 Proof.
