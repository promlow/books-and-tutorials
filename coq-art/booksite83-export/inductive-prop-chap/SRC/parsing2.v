...
Fixpoint bin_to_string (t:bin) : list char :=
  match t with
  | L => nil (A:=char)
  | parsing2.N u v => open :: bin_to_string u ++ close :: bin_to_string v
  end.
...
Inductive parse_rel : list char -> list char -> bin -> Prop :=
  | parse_node :
      forall (l1 l2 l3:list char) (t1 t2:bin),
        ...
        parse_rel l2 l3 t2 -> parse_rel (open :: l1) l3 (parsing2.N t1 t2)
...
apply Hr2 with (s := s') (t := parsing2.N t L).
