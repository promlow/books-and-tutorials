Add LoadPath  "../../inductive-prop-chap/SRC".
...
(* We use the refine tactic and we hope this makes the algorithm
 ...
 refine
   (fun l0 rec =>
      match rec l0 _ with
      | inleft (existS (close::l1) (exist t1 (conj Hp Hl))) =>
        match rec l1 _ with
        | inleft (existS l2 (exist t2 (conj Hp2 Hl2))) =>
          inleft _
      ...
      | inleft (existS (open::l1) (exist t1 (conj Hp Hl))) => 
         inright _ (fun l'' t' Hp' => _)
      | inleft (existS nil (exist t1 (conj Hp Hl))) =>
         inright _ (fun l'' t' Hp' => _)
      | inright Hnp => inright _ (fun l'' t' Hp' => _)
      end);(auto ||
            (try (inversion Hp';
