...
Function filled_leaves (N L:Type)(t:tree N (option L)){struct t} : nat :=
 match t with
   Leaf None => 0
   | Leaf (Some n) => 1
   | Node _ t1 t2 => filled_leaves N L t1 + filled_leaves N L t2
 end.
...
Function insert (L:Type)(l:L)(t: tree bool (option L)) {struct t} :
                             tree bool (option L):=
   match t with 
          Leaf None => Leaf _ _ (Some l)
         |Leaf (Some b) => Node _ _ true (Leaf _ _ (Some b)) 
                                         (Leaf _ _ (Some l))
         |Node  true  t1 t2 =>
...
Function how_many_in_tree (n:nat)(t:tree bool (option nat)){struct t} : nat:=
 match t with
  Leaf None => 0
 |Leaf (Some p) => if eq_nat_dec n p then 1 else 0
 |Node _ t1 t2 =>   how_many_in_tree n t1 +   how_many_in_tree n t2
 end.  
